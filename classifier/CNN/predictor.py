...
def predict_from_frozen(image_path, model):
    ...
    with tf.Session(graph=graph) as sess:
        ...
        results = sess.run(y, feed_dict={
            x: image
        })
        prediction = np.squeeze(results)
        top_k = prediction.argsort()[-3:][::-1]
        
        classification = []
        for i in top_k:
          classification.append((labels[i], prediction[i]))
...
if __name__ == '__main__':
  ...
  image_path = "../../output/test/1.jpg"
  predict_from_frozen(image_path, "inception_v3_2016_08_28")
